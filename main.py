...
# Card-specific fonts for shop
card_title_font = pygame.font.SysFont("Comic Sans MS", 28)
card_desc_font = pygame.font.SysFont("Comic Sans MS", 18)
...
def save_coin_count():
    try:
        ...
        # leaderboard removed: do not persist leaderboard data
        try:
...
while running:
    ...
    # expose mouse position and click state for UI blocks (menu/shop/settings)
    ...
    if game_state == "playing":
        ...
        try:
            ...
            if (finger_x and finger_y and home_rect.collidepoint(finger_x, finger_y)) or (home_rect.collidepoint(mx, my) and mouse_pressed):
                # go back to menu and clear gameplay lists (leaderboard removed)
                game_state = "menu"
                finger_trail.clear()
                fruits.clear()
                halves.clear()
        ...
        if game_state == "menu":
            # Responsive layout: title centered, buttons in the middle column
            title_y = 48
            ...
            # (leaderboard removed; main menu occupies center column)
            ...
            # Define shop items (20) grouped by category via 'category' field.
            shop_items = [
                # Trails
                {"id": "trail_neon", "name": "Trail Neon", "price": 5, "desc": "Neon blue swipe trail", "category": "Trails"},
                {"id": "trail_rainbow", "name": "Trail Rainbow", "price": 8, "desc": "Colorful rainbow swipe trail", "category": "Trails"},
                {"id": "trail_glow", "name": "Trail Glow", "price": 6, "desc": "Soft glowing trail", "category": "Trails"},

                # Skins (hand/icon/knife visuals)
                {"id": "skin_tiger", "name": "Tiger Skin", "price": 12, "desc": "Striped tiger knife look", "category": "Skins"},
                {"id": "skin_pastel", "name": "Pastel Skin", "price": 10, "desc": "Soft pastel swipe visuals", "category": "Skins"},
                {"id": "skin_nebula", "name": "Nebula Skin", "price": 15, "desc": "Cosmic particle skin", "category": "Skins"},
                {"id": "skin_gold", "name": "Gold Skin", "price": 20, "desc": "Shiny gold swipe", "category": "Skins"},
                {"id": "skin_wood", "name": "Wood Skin", "price": 7, "desc": "Warm wooden look", "category": "Skins"},
                {"id": "skin_sketch", "name": "Sketch Skin", "price": 9, "desc": "Hand-drawn sketch effect", "category": "Skins"},

                # Sound Effects
                {"id": "sfx_slash_ex", "name": "Slash FX Pack", "price": 6, "desc": "Extra slicing sounds", "category": "SFX"},
                {"id": "sfx_boom_ex", "name": "Boom FX Pack", "price": 6, "desc": "Louder bomb impacts", "category": "SFX"},
                {"id": "sfx_ui", "name": "UI Clicks", "price": 4, "desc": "Polished menu sounds", "category": "SFX"},

                # Powerups
                {"id": "pw_double", "name": "Double Points", "price": 18, "desc": "Temporarily double points", "category": "Powerups"},
                {"id": "pw_slow", "name": "Slow Motion", "price": 14, "desc": "Slow fruit speed briefly", "category": "Powerups"},
                {"id": "pw_shield", "name": "Shield", "price": 22, "desc": "Protect from one bomb", "category": "Powerups"},
                {"id": "pw_multi", "name": "Multi Slice", "price": 16, "desc": "Split one swipe into three", "category": "Powerups"},

                # Backgrounds
                {"id": "bg_tropical", "name": "Tropical", "price": 8, "desc": "Sunny beach background", "category": "Backgrounds"},
                {"id": "bg_neon_city", "name": "Neon City", "price": 12, "desc": "Futuristic neon skyline", "category": "Backgrounds"},
                {"id": "bg_space", "name": "Space", "price": 15, "desc": "Starfield & planets", "category": "Backgrounds"},
                {"id": "bg_zen", "name": "Zen Garden", "price": 9, "desc": "Calm minimalist garden", "category": "Backgrounds"},
            ]
            ...
            card_h = 130
            spacing = 18
            # show only two cards in the viewport initially to avoid overlap with Back
            visible_h = card_h * 2 + spacing
            area_bottom = area_top + visible_h
            cx = (width - card_w) // 2

            # flatten list, render category headers when category changes
            y = area_top - shop_scroll
            last_cat = None
            for idx, item in enumerate(shop_items):
                # draw category header when needed
                cat = item.get("category", None)
                if cat and cat != last_cat:
                    hdr = small_font.render(cat, True, (255, 230, 180))
                    hdr_y = y - 18
                    # only draw category header if it falls within the visible scroll area
                    try:
                        if hdr_y + hdr.get_height() >= area_top and hdr_y <= area_bottom:
                            screen.blit(hdr, (cx, hdr_y))
                    except Exception:
                ...
                if rect.bottom >= area_top and rect.top <= area_bottom:
                    # shadow (soft offset)
                    try:
                        shadow_surf = pygame.Surface((rect.w, rect.h), pygame.SRCALPHA)
                        pygame.draw.rect(shadow_surf, (0, 0, 0, 90), shadow_surf.get_rect(), border_radius=12)
                        screen.blit(shadow_surf, (rect.x + 4, rect.y + 6))
                    except Exception:
                        pass
                    ...
                    try:
                        ...
                        pygame.draw.rect(panel_surf, (28, 30, 34), panel_surf.get_rect(), border_radius=12)
                        # subtle inner highlight
                        try:
                            inner = panel_surf.get_rect().inflate(-6, -6)
                            pygame.draw.rect(panel_surf, (36, 38, 42), inner, border_radius=10)
                        except Exception:
                            pass
                    ...
                    # icon placeholder (filled circle) on left with initials
                    icon_r = 30
                    icon_x = rect.x + 16
                    icon_y = rect.y + (rect.h // 2)
                    try:
                        # outer circle (border)
                        pygame.draw.circle(screen, (40, 40, 40), (icon_x + icon_r, icon_y), icon_r + 2)
                        # filled inner circle
                        pygame.draw.circle(screen, (80, 80, 80), (icon_x + icon_r, icon_y), icon_r)
                        # draw initials (1-2 letters) centered
                        name = item.get("name", "")
                        initials = "".join([w[0] for w in name.split()[:2]]).upper()
                        if not initials:
                            initials = item.get("id", "?")[:2].upper()
                        init_surf = card_title_font.render(initials, True, (220, 220, 220))
                        ix = icon_x + icon_r - init_surf.get_width() // 2
                        iy = icon_y - init_surf.get_height() // 2
                        screen.blit(init_surf, (ix, iy))
                    except Exception:
                        pass

                    # vertically center icon, text and button to align like mockup
                    content_cy = rect.y + rect.h // 2

                    # item title and description (use card fonts, aligned to center Y)
                    it_name = card_title_font.render(item["name"], True, (255, 240, 180))
                    it_x = rect.x + 16 + (icon_r * 2) + 12
                    # nudge title slightly up and description a bit lower to avoid overlap
                    it_y = content_cy - 18
                    screen.blit(it_name, (it_x, it_y))
                    it_desc = card_desc_font.render(item.get("desc", ""), True, (200, 200, 200))
                    screen.blit(it_desc, (it_x, content_cy + 10))

                    # combined price+button: show price and Buy/Select in one compact control
                    owned = (item["id"] in purchased_items)
                    btn_w_local = 96
                    btn_h_local = 36
                    btn_rect = pygame.Rect(rect.right - 12 - btn_w_local, content_cy - (btn_h_local // 2), btn_w_local, btn_h_local)
                    ...
                    # render price inside the button (left) and label on right
                    try:
                        price_txt = tiny_font.render(str(item['price']), True, (255, 255, 255))
                        label_txt = tiny_font.render("Select" if owned else "Buy", True, (255, 255, 255))
                        # price at left inside button with small padding
                        px = btn_rect.x + 8
                        py = btn_rect.y + (btn_rect.h - price_txt.get_height()) // 2
                        screen.blit(price_txt, (px, py))
                        # label at right inside button with padding
                        lx = btn_rect.x + btn_rect.w - label_txt.get_width() - 8
                        ly = btn_rect.y + (btn_rect.h - label_txt.get_height()) // 2
                        screen.blit(label_txt, (lx, ly))
                    except Exception:
                        pass

                    # handle touch/click (mouse or finger)
                    if (finger_x and finger_y and btn_rect.collidepoint(finger_x, finger_y)) or (btn_rect.collidepoint(mx, my) and clicked):
                        if not owned:
                            ...
                            if item["id"].startswith("trail_"):
                                ...
                                shop_msg = {"text": f"Owned: {item['name']}", "age": 0, "life": 60}

                # advance y for next item
                y += card_h + spacing

            # clamp shop_scroll so user can't scroll past content bottom
            try:
                total_h = y - (area_top - shop_scroll)
                max_scroll = max(0, total_h - visible_h)
                shop_scroll = max(0, min(shop_scroll, max_scroll))
            except Exception:
                shop_scroll = max(0, shop_scroll)

            # draw Shop title on top so it never gets covered by cards
            try:
    ...
    try:
        if game_over_active or game_state == "gameover":
            ...
            # GAME OVER text: apply a "jelly" (wobble/squash) text-only effect
            try:
                go_text = combo_font.render("GAME OVER", True, (255, 255, 255))
                sub = small_font.render("You hit a bomb!", True, (255, 230, 230))

                # Jelly effect window: from 2.5s until the end of the game-over duration
                jelly_start = 2.5
                if elapsed >= jelly_start and elapsed <= dur:
                    # normalized t in [0,1] across the jelly window
                    t_j = (elapsed - jelly_start) / float(max(1e-6, (dur - jelly_start)))

                    # time-based oscillation for organic wobble
                    now_t = time.time()
                    freq = 10.0 + 14.0 * (1.0 - t_j)  # faster wobble earlier
                    wob = math.sin(now_t * freq) * (1.0 - t_j)

                    # horizontal/vertical squash to create a jelly feel
                    scale_x = 1.0 + 0.14 * wob
                    scale_y = 1.0 - 0.14 * wob

                    # prepare scaled surface
                    bw, bh = go_text.get_size()
                    sw = max(1, int(bw * scale_x))
                    sh = max(1, int(bh * scale_y))
                    try:
                        scaled = pygame.transform.smoothscale(go_text, (sw, sh)).convert_alpha()
                    except Exception:
                        scaled = go_text

                    # alpha fade same as overlay
                    try:
                        scaled.set_alpha(max(120, alpha))
                    except Exception:
                        pass

                    # subtle positional jitter while wobbling
                    jitter_x = int(math.sin(now_t * (freq * 0.9)) * 8 * (1.0 - t_j))
                    jitter_y = int(math.cos(now_t * (freq * 0.7)) * 6 * (1.0 - t_j))

                    rect = scaled.get_rect(center=(width // 2 + jitter_x, height // 2 - 20 + jitter_y))
                    screen.blit(scaled, rect.topleft)

                    # draw subtext with small horizontal nudge (no other visuals)
                    sub_x = (width - sub.get_width()) // 2 + int(math.cos(now_t * 7.0) * 4 * (1.0 - t_j))
                    screen.blit(sub, (sub_x, rect.bottom + 6))
                else:
                    # default static text
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False
        # Mouse wheel for scrolling the shop (and general fallback for other lists)
        elif event.type == pygame.MOUSEWHEEL:
            try:
                # wheel up -> event.y positive on many platforms
                shop_scroll = max(0, shop_scroll - int(event.y * 40))
            except Exception:
                pass
            ...
            try:
                ...
                if event.button == 4:
                    shop_scroll = max(0, shop_scroll - 40)
                elif event.button == 5:
                    shop_scroll += 40
            except Exception:
                pass
